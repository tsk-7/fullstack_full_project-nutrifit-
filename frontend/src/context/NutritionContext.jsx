...
import axiosInstance, { authAPI } from '../services/api';
...
export const NutritionProvider = ({ children }) => {
    ...
    const getAuthToken = (tokenValue = jwtToken) => getTokenString(tokenValue);

    const getAuthConfig = (tokenValue = jwtToken) => {
    ...
    const fetchDoctors = async () => {
        try {
            const response = await axiosInstance.get('/doctors', getAuthConfig(jwtToken));
            const data = response.data;
            if (!Array.isArray(data)) throw new Error('Invalid doctors response format');
            setDoctors(data);
        } catch (err) {
            const errorMsg = getApiErrorMessage(err, 'Failed to load doctors');
    ...
    const fetchUserProfile = async () => {
        ...
        try {
            const response = await axiosInstance.get('/users/me', getAuthConfig(jwtToken));
            const data = response.data;
            if (!data.id) throw new Error('Invalid profile response format');
            setUserProfile(data);
        } catch (err) {
            const errorMsg = getApiErrorMessage(err, 'Failed to fetch profile');
    ...
    const fetchTodayMeals = async () => {
        if (!userId || !jwtToken) return;
        try {
            const response = await axiosInstance.get('/meals/today', getAuthConfig(jwtToken));
            const data = response.data;
            ...
            const errorMsg = getApiErrorMessage(err, 'Failed to fetch meals');
    ...
    const fetchMealTotals = async () => {
        ...
        try {
            const response = await axiosInstance.get('/meals/totals/today', getAuthConfig(jwtToken));
    ...
    const fetchTodayWater = async () => {
        if (!userId || !jwtToken) return;
        try {
            const response = await axiosInstance.get('/meals/water/today', getAuthConfig(jwtToken));
            const data = response.data;
    ...
    const updateWaterGlasses = async (glasses, consumedOn = null) => {
        ...
        try {
            const response = await axiosInstance.put('/meals/water', { glasses, consumedOn }, getAuthConfig(jwtToken));
            const data = response.data;
            setWaterGlasses(Number(data?.glasses || 0));
            setError(null);
            return true;
        } catch (err) {
            const errorMsg = getApiErrorMessage(err, 'Failed to update water logs');
    ...
    const getDailyReport = async (date = null) => {
        if (!userId || !jwtToken) return null;
        try {
            const response = await axiosInstance.get('/meals/report/daily', {
                ...getAuthConfig(jwtToken),
                params: date ? { date } : undefined
            });
            setError(null);
            return response.data;
        } catch (err) {
            const errorMsg = getApiErrorMessage(err, 'Failed to fetch daily report');
    ...
    const getDoctorUserDaywiseReport = async (targetUserId, from = null, to = null) => {
        ...
        try {
            const response = await axiosInstance.get(`/meals/doctor/user/${targetUserId}/daywise`, {
                ...getAuthConfig(jwtToken),
                params: { ...(from ? { from } : {}), ...(to ? { to } : {}) }
            });
            return response.data;
        } catch (err) {
            const errorMsg = getApiErrorMessage(err, 'Failed to fetch user day-wise report');
    ...
    const updateProfile = async (profileData) => {
        ...
        try {
            ...
            const response = await axiosInstance.put('/users/me/profile', normalizedPayload, getAuthConfig(jwtToken));
            const data = response.data;
            if (!data.id) throw new Error('Invalid profile response format');
            setUserProfile(data);
            return true;
    ...
    const sendMessage = async (doctorId, text) => {
        if (!userId || !jwtToken) return false;
        try {
            const response = await axiosInstance.post('/messages', { doctorId, text }, getAuthConfig(jwtToken));
            const newMessage = mapMessageForUi(response.data);
    ...
    const sendDoctorMessage = async (doctorId, targetUserId, text) => {
        ...
        try {
            const response = await axiosInstance.post('/messages', { userId: Number(targetUserId), text }, getAuthConfig(jwtToken));
            const newMessage = mapMessageForUi(response.data);
    ...
    const rateMessage = async (doctorId, messageId, rating, feedback) => {
        if (!userId || !jwtToken) return false;
        try {
            await axiosInstance.put(`/messages/${messageId}/rate`, { rating, feedback }, getAuthConfig(jwtToken));
            // Update local state
            const messageKey = `user_${userId}_doctor_${doctorId}`;
            setChatMessages(prev => ({
                ...prev,
            ...
            const errorMsg = getApiErrorMessage(err, 'Failed to rate message');
    ...
    const getMessages = async (doctorId) => {
        if (!userId || !jwtToken) return [];
        try {
            const response = await axiosInstance.get(`/messages/conversation/${doctorId}`, getAuthConfig(jwtToken));
            ...
            const errorMsg = getApiErrorMessage(err, 'Failed to fetch messages');
    ...
    const getDoctorInbox = async () => {
        ...
        try {
            const response = await axiosInstance.get('/messages/doctor/inbox', getAuthConfig(jwtToken));
            const data = response.data;
            if (!Array.isArray(data)) throw new Error('Invalid inbox response format');
            setError(null);
            return data;
        } catch (err) {
            const errorMsg = getApiErrorMessage(err, 'Failed to fetch inbox');
    ...
    const getDoctorConversation = async (targetUserId) => {
        ...
        try {
            const response = await axiosInstance.get(`/messages/doctor/conversation/${targetUserId}`, getAuthConfig(jwtToken));
            ...
            const errorMsg = getApiErrorMessage(err, 'Failed to fetch conversation');
    ...
    const addMeal = async (meal) => {
        if (!userId || !jwtToken) return false;
        try {
            const response = await axiosInstance.post('/meals', meal, getAuthConfig(jwtToken));
            const newMeal = mapMealForUi(response.data);
            ...
            const errorMsg = getApiErrorMessage(err, 'Failed to add meal');
    ...
    const removeMeal = async (mealId) => {
        if (!userId || !jwtToken) return false;
        try {
            await axiosInstance.delete(`/meals/${mealId}`, getAuthConfig(jwtToken));
            setMealsData(prev => ({
                ...prev,
            ...
            const errorMsg = getApiErrorMessage(err, 'Failed to remove meal');
